...
const sections: Dictionary<ISectionDefinition> = {
  ...
  MidiMessage: {
    ...
    options: [
      ...
      {
        value: ButtonMessageType.SaxPitchBendCenterCapture,
        text: "PB Center Capture (캘리브레이션)",
      },
  ...
  - PB Center Capture: 현재 피치벤드 값을 ‘중앙값(0)’으로 캡처해 장치에 저장합니다(전원을 꺼도 유지).

PB Center Capture 사용법(추천):
  1) 평소 연주할 때처럼 ‘입으로 무는 정도’를 유지
  2) 그 상태에서 버튼을 한 번 눌러 캡처
  3) 이후부터는 그 ‘무는 상태’가 0(센터) 기준이 되고, 입을 떼면 음의 방향(−쪽)으로 자연스럽게 내려갑니다.
